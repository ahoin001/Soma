      ...
      const micros = await client.query(
        ...
        SELECT
          COALESCE(SUM((COALESCE(
            (item.micronutrients->>'sodium_mg')::numeric,
            (foods.micronutrients->>'sodium_mg')::numeric,
            0
          ) * COALESCE(item.quantity, 1))), 0) AS sodium_mg,
          COALESCE(SUM((COALESCE(
            (item.micronutrients->>'fiber_g')::numeric,
            (foods.micronutrients->>'fiber_g')::numeric,
            0
          ) * COALESCE(item.quantity, 1))), 0) AS fiber_g,
          COALESCE(SUM((COALESCE(
            (item.micronutrients->>'sugar_g')::numeric,
            (foods.micronutrients->>'sugar_g')::numeric,
            0
          ) * COALESCE(item.quantity, 1))), 0) AS sugar_g,
          COALESCE(SUM((COALESCE(
            (item.micronutrients->>'potassium_mg')::numeric,
            (foods.micronutrients->>'potassium_mg')::numeric,
            0
          ) * COALESCE(item.quantity, 1))), 0) AS potassium_mg,
          COALESCE(SUM((COALESCE(
            (item.micronutrients->>'cholesterol_mg')::numeric,
            (foods.micronutrients->>'cholesterol_mg')::numeric,
            0
          ) * COALESCE(item.quantity, 1))), 0) AS cholesterol_mg,
          COALESCE(SUM((COALESCE(
            (item.micronutrients->>'saturated_fat_g')::numeric,
            (foods.micronutrients->>'saturated_fat_g')::numeric,
            0
          ) * COALESCE(item.quantity, 1))), 0) AS saturated_fat_g
        FROM meal_entry_items item
        JOIN meal_entries ON meal_entries.id = item.meal_entry_id
        LEFT JOIN foods ON foods.id = item.food_id
        WHERE meal_entries.user_id = $1 AND meal_entries.local_date = $2;
        `,
        [userId, localDate],
      );
